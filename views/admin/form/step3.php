...
<section>
    <div id="kemampuan-content" class="step-3-content">
        <!-- Kompetensi Content Start -->
        <div class="mb-5">
            <label for="label-content">
                Pada saat lulus, pada tingkat mana kompetensi di bawah ini anda kuasai? (A)
                Pada saat ini, pada tingkat mana kompetensi di bawah ini diperlukan dalam pekerjaan? (B) <span
                            ...
                            <tr>
                                ...
                                <?php for ($i = 1; $i <= 5; $i++): ?>
                                    <td><label><input type="radio" class="form-check-input" name="<?= $kodeA ?>"
                                                value="<?= $i ?>" <?php echo ($data[$kodeA] == $i) ? 'checked' : 'disabled'; ?> /> </label></td>
                                <?php endfor; ?>
                                ...
                                <?php for ($i = 1; $i <= 5; $i++): ?>
                                    <td><label><input type="radio" class="form-check-input" name="<?= $kodeB ?>"
                                                value="<?= $i ?>" <?php echo ($data[$kodeB] == $i) ? 'checked' : 'disabled'; ?> /> </label></td>
                                <?php endfor; ?>
                            </tr>
                        <?php endforeach; ?>
                    </tbody>
                        ...
                        <?php foreach ($opsi as $value => $label): ?>
                            <div class="form-check mb-2"> <!-- Bootstrap form-check -->
                                <input class="form-check-input" type="radio" name="<?= $key ?>" value="<?= $value ?>"
                                    id="<?= $key . $value ?>"
                                    <?php echo (isset($data[$key]) && $data[$key] == $value) ? 'checked' : 'disabled'; ?>>
